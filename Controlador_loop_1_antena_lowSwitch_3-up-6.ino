//CODIGO PARA EL CONTROL DE 1 MAGNETIC LOOP ANTENNAS
//ESCRITO POR JOSÉ BASCÓN EA7HVO
//LICENCIA PARA USO NO COMERCIAL
//pololu is now in pin 2
//direction is now in pin 5
//LowSwitch is nonw in pin 3
//upSwitch is now in pin 6
...
#define lowSwitch 3   // pin 12 para final de carrera inferior
#define upSwitch 6   //pin 13 para fin de carrera superior
...
int direccion = 5;  //toma el valor de el pin 5 para el pololu 1 DIR del pololu 
int pololu = 2;    //toma el valor de el pin 2 STEP del pololu  
  ...
  if(digitalRead(UP)==LOW && digitalRead(DOWN)==LOW)   //reset encoderValue si pulsamos pin 17 y 11
              {
                  ...
                  delay(3000); 
                  upperlimit=64000;                                  
        for (i=mempointer; i <= mempointer+90; i++) {
                  EEPROM.put(i,0);     //escribimos el valor o en todas las posiciones
                  }  
                  lcd.clear();
                  encoderValue=1;                                   
